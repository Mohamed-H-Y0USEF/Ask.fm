...
struct QuestionsManager {
	...
	int getId(Question &Q){
		int id = 0;
		for(int i = 0 ; i < (int) Q.question_text.size() ; i++){
			id += i * Q.question_text[i];
		}
		id += Q.from_user_id + Q.to_user_id;
		return id;
	}
	QuestionsManager() {
	...
	void LoadDatabase() {
		...
		for (auto &line : lines) {
			...
			if (question.parent_question_id == -1)
				questionid_questionidsThead_to_map[question.question_id].push_back(question.question_id); // head of the thread
			else
		...
		for (auto &pair : questionid_questionidsThead_to_map) {	// pair<int, vector<Question>>
			for (auto &question_id : pair.second) {		//  vector<Question>
				...
				if (question.to_user_id == user.user_id) {
					if (question.parent_question_id == -1)
						user.questionid_questionidsThead_to_map[question.question_id].push_back(question.question_id); // first one in the thread
					else
	...
	void AnswerQuestion(User &user) {
		...
		// because the dilemater in files in a comma
	...
	void AskQuestion(User &user, pair<int, int> to_user_pair) {
		...
		question.question_id = getId(question);
...
struct UsersManager {
	...
	void DoSignUp() {
		...
		current_user.user_id = getId(current_user);
